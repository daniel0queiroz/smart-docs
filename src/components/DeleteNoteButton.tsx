...
function DeleteNoteButton({ noteId, deleteNoteLocally }: Props) {
    ...
    const handleDeleteNote = () => {
        ...
        if (!errorMessage) {
          ...
          description: "Você deletou a anotação.",
        });
  ...
  <AlertDialogContent>
    <AlertDialogHeader>
      <AlertDialogTitle>Você tem certeza que deseja deletar esta anotação?</AlertDialogTitle>
      <AlertDialogDescription>
        Esta ação não poderá ser desfeita. Isto irá deletar sua anotação permanentemente.
      </AlertDialogDescription>
    </AlertDialogHeader>
    <AlertDialogFooter>
      ...
      >{isPending ? <Loader2 className="animate-spin" /> : "Deletar"}</AlertDialogAction>
    </AlertDialogFooter>
  </AlertDialogContent>
</AlertDialog>
  )
