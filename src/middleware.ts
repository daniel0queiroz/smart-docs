...
export async function middleware(request: NextRequest) {
  ...
  if (!searchParams.get("noteId") && pathname === "/") {
    ...
    if(user) {
      const {newesNoteId} = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/fetch-newest-note?userId=${user.id}`).then((res) => res.json());

      if (newesNoteId) {
        const url = request.nextUrl.clone()
        url.searchParams.set("noteId", newesNoteId)
